...
export function createKeptTestsHandlers(options: KeptTestsHandlerOptions): KeptTestsHandlers {
	...
	return {
		...
		async set_kept_tests(params: SetKeptTestsParams): Promise<SetKeptTestsResult> {
			let held: KeptTest[];
			try {
			...
			if (next.length > MAX_KEPT_TESTS) {
				// Nothing is written and nothing is dropped to make room — the store is exactly as
				// it was, which is what makes this a refusal an operator acts on rather than a
				// silent eviction of somebody else's exemption.
