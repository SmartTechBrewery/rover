...
- **The brief that produced these screens is [`docs/DESIGN_INITIAL_PROMPT.md`](../docs/DESIGN_INITIAL_PROMPT.md)**, and the running list of what the panel eventually needs is [`docs/WEB_PANEL.md`](../docs/WEB_PANEL.md). Both explain *why* a screen looks the way it does — in particular that Rover is not a test framework, so no pass/fail badge, status colour or success rate belongs anywhere in this UI,. **Both still describe the panel as read-only, and that is no longer true**: it performs operator actions, force-releasing a stuck lease being the first. #106 corrects both documents and records what the panel is allowed to do; until it lands, treat their read-only framing as stale rather than as a constraint to design against.
