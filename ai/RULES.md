...
**Where that line falls was settled on 2026-09-10, and this paragraph is edited in place with its reasoning extended rather than replaced.** The sentence above was being read as *Rover may not compute anything about what it captured*, and three documents cited it for a specific prohibition — the web panel may not diff two screenshots (`docs/DESIGN_INITIAL_PROMPT.md` §4, `docs/DESIGN.md` §9, `docs/WEB_PANEL.md` item 9, and `PROJECT.md` R41's closing clause). That reading is wider than the rule. **Locating a difference is not judging one.** *These two files differ in this rectangle* is arithmetic over two artifacts, of the same class as their size on disk or their pixel dimensions, and it is the part of a comparison a machine does better than a person; withholding it did not make Rover more honest, it just made the reader do it by eye.

So the rule is about **verdicts, not about arithmetic**, and it is unchanged in everything it was actually protecting. Still forbidden, everywhere, in every surface: a pass, a fail, a score, a percentage, a similarity figure, a threshold anything is measured against, a baseline arm and a current one, a red/green pairing, a ranking of one finding over another, and anything that goes red on its own. **Rover may state what it measured; it may never say whether that is good.** A feature that computes something has to be able to say which side of that line it is on in one sentence — and if the sentence needs a paragraph, it is on the wrong side.
...
**Keep `PROJECT.md`, `README.md` and `docs/MANUAL.md` current.** The README is the short version — what Rover is, what you get, and how to install it — and [`docs/MANUAL.md`](../docs/MANUAL.md) is everything else that used to live below it: the walk-throughs, the configuration reference, the hooks, the archive, the panel and the state of what is built. If a change makes any of them inaccurate — a renamed verb, a changed lease rule, a decision reversed — update it in the same change. A reversed decision is edited **in place with its reason rewritten**, not deleted: the record of what was considered and rejected is most of that document's value.

**A feature that lands, changes or goes is written into [`docs/FEATURES.md`](../docs/FEATURES.md) in the same change.** That file is the catalogue every user-facing text is derived from — the README's one-sentence hooks, the manual's sections, an issue's motivation paragraph, the `ROVER.md` an agent reads before its first call — so it is the one document whose drift is invisible until somebody writes prose from it and gets the product wrong. Add a section for a new feature, rewrite the affected one **in place with its reasoning** when behaviour changes, and say so there when something is removed; a feature that is only planned belongs in `PROJECT.md` §9's backlog and never here. Every entry carries the same three parts — the one sentence a reader would recognise as a reason to use Rover, the long description including *why* it works that way, and where the code and the decisions live — and a feature whose *hook* you cannot write in one sentence is usually a feature that has not been settled.
...
Also give every issue a type label (`bug`, `enhancement`, `feature`) and, where it helps, a component label (`daemon`, `core`, `backend`, `cli`, `mcp`, `docs`, `panel`).
...
Rover reads configuration from a per-project file (the hooks of `PROJECT.md` D13: install command, helper services, teardown) and from environment variables for host-level settings (lease TTL, socket path, log level). That file is `<project>.json` under `ROVER_PROJECTS_PATH` (`~/.rover/projects` by default), on the **host** — `src/daemon/project-hooks.ts` is its schema and its reader. It carries `project`, `apps`, `install`, `services` and `teardown`, and each field landed with its consumer; there is deliberately no port field, because allocating one per lease is R18. A **client** may be pointed at one such file by `ROVER_PROJECT_FILE` and reads a single field out of it — the `project` that then defaults `acquire`'s attribution string (D22); it reads nothing else and runs nothing a file declares.
...
**Read Stitch, never write it.** `generate_screen_from_text`, `edit_screens`, `generate_variants`, `create_project` and `delete_project` are **not** for an agent to call. Designing is the operator's round trip: an agent's deliverable is the **prompt**, complete and ready to paste, and the person at the keyboard submits it and judges what comes back. Two things make this a rule rather than a preference — Stitch's report of what it changed is sometimes simply false (`docs/DESIGN.md` §11), so the result needs eyes that are not the agent's; and **a screen cannot be deleted**, only the whole project can, so an unattended generation spends the project's namespace permanently.
...
- **Read [`docs/DESIGN.md`](../docs/DESIGN.md) before touching any panel screen.** It carries what the Devices screen settled — the shell, the breadcrumb rule, the LED vocabulary, the card anatomy, the layout rules, what is deliberately absent, and the traps that cost time working through Stitch itself. A screen built without it re-derives all of that and gets some of it wrong.
- **The brief that produced these screens is [`docs/DESIGN_INITIAL_PROMPT.md`](../docs/DESIGN_INITIAL_PROMPT.md)**, and the running list of what the panel eventually needs is [`docs/WEB_PANEL.md`](../docs/WEB_PANEL.md). Both explain *why* a screen looks the way it does — in particular that Rover is not a test framework, so no pass/fail badge, status colour or success rate belongs anywhere in this UI. **That list is a list of verdicts and it stands unchanged**; §1 records where the line between a verdict and a measurement was settled, and the comparison card's difference marks are the one feature that has had to sit on it — measured, off by default, named in words, and carrying no score of any kind (`docs/DESIGN.md` §9). **Neither describes the panel as read-only any more**, and it is not: it performs operator actions, force-releasing a stuck lease being the first one built (D27, `docs/DESIGN.md` §2 and §7). Both documents were corrected on 2026-08-31 and say so at the top; what the panel is allowed to do is D27's, not either document's.
