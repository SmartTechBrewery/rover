...
- **The grouping is the archive's third read, and again its own row** (R41, #178).
  ...
  bounded like the search and for the same reason — capped by directories read and by three
  structural caps, with `truncated` meaning *at least one directory that exists was not fully
  examined* — and it answers `list_archive`'s own three outcomes, so *no groups here* and *the host
