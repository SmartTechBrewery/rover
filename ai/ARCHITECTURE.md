...
**Enumeration answers what can be borrowed now, not what the machine could run** (`PROJECT.md` D41). One platform's own device list already answers it that way — an unplugged phone stops being listed — and another's lists everything ever created, so the backend narrows it: a virtual device nobody has started is not a device this host has. That is a rule about **presence**, never about the state: a device that is attached and unusable is still enumerated and still says so, and the row saying `unauthorized` is the only clue its operator gets about why nothing can be leased on it. And it is a rule about the **inventory** — enumeration and the change stream. A question asked about one *named* device (`describeDevice`, `deviceInfo`) is asked by a caller who already has it in mind, so it answers what that device is, which is what lets a lease grant name a state rather than only an absence.
...
That last parenthesis is about `simctl`, not about the platform — corrected 2026-09-08. `idb_companion --notify` streams the **full current target set on every change**, which is `DeviceWatcher.onDevices`' contract to the letter, so an iOS backend that takes the idb dependency does not poll either. **That is now the shipped arrangement rather than a prediction** (#249): `IosSimulatorDeviceBackend.watchDevices` runs on the stream and keeps the `simctl list` poll as its fallback, which is the third shape this interface has to admit and does — **one backend, two sources, one contract**, with only one of them delivering at a time. The interface requirement stands unchanged and needed no edit for it: it may not *assume* either shape, and a backend switching between them behind `onDevices`/`onInterrupted` is not something a caller can see.
...
- **Restoration runs on release, on force-release, on expiry *and* on the host's own shutdown** (D9): stop a recording the lease left running and remove its file, stop the app, airplane mode off, wifi on, stop the project's helper services, then the project's teardown hook. The recorder step is first because a recorder is the driver most likely to still be holding the device, and its bytes are dropped rather than archived — a lease that ended has no caller left to hand a recording to. A teardown that only runs on the happy path is not a teardown. Every step is contained, including working out what the project owns, so one unreadable project description costs that project's steps rather than the device's. The teardown hook is foreign code and is bounded like the shutdown's other waits, because a grant queues behind it. **The host's own shutdown is a fourth trigger on that same path** (D9 as amended, #294), and this sentence is edited in place with its reasoning extended rather than replaced (ai/RULES.md §1). It said only that `close()` sweeps once more and waits out what is still owed, which left the leases that had *not* expired unaccounted for. `close()` takes that final expiry sweep and then releases **every lease still live**, through the same `LeaseStore.release` and the same restorer chain a caller's own release uses, before `settleRestorations` is snapshotted — so the shutdown waits for what it started. The argument is the one the sweep already rests on, carried one step further: a lease dies with the host, so a lease with nineteen minutes left has no holder who will ever release it (the host it would call is this one) and no successor who will ever see it expire, and the device is left in whatever state the lease put it in — airplane mode on, a recorder running, the project's services up. It was measured rather than assumed (`PROJECT.md` §6). What made it worth fixing rather than noting is `bin/rover-server-agent`: a host under launchd is stopped routinely — at logout, at reboot, on every `reload` — where a foreground one was stopped by a person who was watching it. Waiting is still bounded, and an abandoned restoration is never retried by anything.
...
- **The answer has three branches, all data** (`src/ipc/verb-methods.ts`). `ok` carries the
  ...
  as `internal_error`, which keeps that code meaning "the host broke". A *grant*'s refusal
  vocabulary is its own and one wider — `held`, `gone`, `not-attached`, `not-ready`,
  `service-failed` — the last being a helper service the lease's project declares that would not
  start (see "Project hooks").
...
- **The grouping is the third method that reads the archive, and again its own row** (R41, #178).
  `src/daemon/list-archive-groups.ts` answers `list_archive_groups` — no parameter at all — with
  which groups exist (a `(project, groupId)` pair read out of each run's `group_id.json`), which
  runs are in each as the components `list_archive` would name them, and which of a grouped run's
  artifacts carry a label together with **the label as the archive filed it**: `pathSegment` ran on
  the way in and is not reversible, so the caller's own string is unrecoverable and is never
  presented as one. That label is decoded from the artifact's own file name by `filedLabelOf`, the
  inverse of the `labelled` that wrote it — both in `src/daemon/archive-path.ts`, so one module owns
  the layout in both directions. **Nothing about what the archive writes changed to serve it**: no
  sidecar file, no `<group_id>/` level, `leaseArchiveDirectory` still always four levels. It is
  bounded like the search and for the same reason — capped by directories read, by three
  structural caps and by a fourth on the answer as a whole (`MAX_ARCHIVE_GROUP_ENTRIES`, since the
  three structural ones bound each level and not their product, and an answer past
  `MAX_FRAME_BYTES` reaches its caller as a *malformed frame* rather than a large one), with
  `truncated` meaning *at least one directory that exists was not fully examined* — and it answers
  `list_archive`'s own three outcomes, so *no groups here* and *the host
  cannot read the archive* never render alike (D6). On the panel's transport and deliberately not
  an MCP tool.
- **What this host is *configured* to do is its own read, beside those two** (R39, D31).
  ...
  **that it will not parse**, which is a different arm of the union from a project that declares
  ...
  transport and deliberately not an MCP tool. **Nothing on this surface creates, edits or renames a
  hook file** — the registrations are the host operator's own, and the one write that does exist is
  a *removal*, `delete_project` (D42), see *Project hooks* below.
- **Which of the archive's tests the operator keeps is a read *and the first write on this surface
  outside a lease*** (D33, #234). `src/daemon/kept-tests-handlers.ts` answers `list_kept_tests`
  with the whole set and `set_kept_tests` with the set after however many tests one press stood
  for — a group's tick is **one** call, so nine tests are one request, one write and one
  authoritative answer. The store is `~/.rover/kept-tests.json`, beside `users.json` and
  deliberately **not** under `artifactsRoot`: the artifact tree is what past leases wrote and every
  sidecar in it goes out once with `flag: 'wx'`, while this toggles (`PROJECT.md` §10). It obeys
  the archive reads' rule about paths the same structural way — no result field a host path would
  fit in, so an unusable store is `unreadable`/`unwritable` with the diagnosis warned on the host
  (D19). Because it is the surface's one non-lease write, it is also where the surface's
  concurrency rule is stated: `src/ipc/server.ts` dispatches frames without awaiting them, so the
  handler **serialises the read-modify-write per store path** and each write renames a uniquely
  named temporary into place — two overlapping presses both land and both are answered with the
  set that includes both, rather than one being silently dropped or the document corrupted. That
  is a lock on a file, not a cache: every call still reads the store and the daemon holds none of
  it (D6). Both rows are on the panel's transport and reachable from the CLI, and both are
  deliberately not MCP tools — what the operator keeps is not an agent's to decide (D27, D28).
...
**Where it lives.** One file per project, `<project>.json` under `ROVER_PROJECTS_PATH` (`~/.rover/projects` by default), beside `rover.sock`, `users.json` and `artifacts/`. It is **host-side operator configuration**: verbs run where the hardware is (D19), and a teardown stranded on the client's machine could not stop the helper service it started. A lease carries an opaque `project` *string* (D22), and **no IPC method creates, edits or renames a hook file, and none accepts a path into that directory** — D31 narrows D13's never-over-the-wire clause to the write and does not repeal it, because every command in the file is a program the host spawns as the daemon's own user, and D27 records that until a role model exists every named user may perform every panel action. **One write does exist and it is a *removal*** (D42, amended in place here the way D31 was): `delete_project` (`src/daemon/delete-project.ts`) unlinks **one** hook file, by identifier and never by path, together with that project's own archive subtree and its kept-test entries. The code-execution reason above is what keeps the other three shut and does not transfer to a delete — it makes the host run strictly less and names no program at all — and the privilege reason is answered by the request being *named and bounded* rather than by D27's deferred role model. **So two methods touch this root, one reading and one deleting**: `list_projects` (R39, `src/daemon/list-projects.ts`) answers which projects are registered here, by name, with no `env` value and no host path on the answer and no field either would fit in (D19), and `delete_project` answers which halves went, with the same promise about its answer. **Three methods now delete from the artifact archive without being a retention bound, and they share one addressed path** (D43, #272, #277 — amended in place here as D31 and D42 are above): `delete_project`'s archive half, `delete_archived_test` (`src/daemon/delete-archived-test.ts`, one test's directory, every run filed under it and its kept-test entry) and `delete_archived_group` (`src/daemon/delete-archived-group.ts`) all go through `ArchiveSweeper.remove(address)`, so the deletion's containment check, its measurement, its per-root serialisation and its `settle()` are one implementation rather than one per level — and the kept-store prune all three run is one routine inside `set_kept_tests`' own write lock. **What the third adds is that its scope is not an address at all**: a group is a claim a run filed and the archive has no level for it (R41), so it walks one project bounded, matches `group_id.json` against the id, and *composes* an address per matched run — `[project, testName, run]`, one component deeper than anything before it — which is what keeps the shared path shared rather than forking a second deletion for a scope no path can name. The projects root is untouched by either finer scope: neither a test nor a group has a registration, which is why a request that reached nothing there answers `not-found` rather than `not-registered`. **A live lease on the project refuses the delete outright and nothing at all is touched**, and the reason is this section's own: the hook file is where that lease's `teardown` and its service stops live, and D9 runs a teardown on release *and* on expiry — deleting the file under a live lease would take the teardown away before the lease that is owed it has ended, which is the teardown-on-the-happy-path-only `ai/RULES.md` §2 forbids. Nothing creates one: registering is still `rover init`'s alone. The file is selected by that string, **re-read at every use and never cached** (D6), and a string that is not a valid identifier resolves to no hooks at all — which is also the traversal guard, since no path is ever built from one that failed the shape.

**What it carries.** `project` (which must equal the file's own name), `apps` — the applications a lease on this project drove — two commands, `install` and `teardown`, and `services`, the named helper processes a lease on this project brings up. Every command is declared as a program and its arguments rather than a shell line. Nothing is defaulted and `services` defaults to empty: a default here would be the core naming an application, which is the bug D13 exists to rule out. `src/daemon/project-hooks.ts` is the Zod schema and the reader; `docs/MANUAL.md`'s configuration section mirrors it. **No port field, deliberately** — allocating one per lease is R18, and a field ahead of its consumer is a catalogue row describing something nothing reads (`ai/RULES.md` §7).

**What a client does with one.** Exactly one thing: reads the `project` field out of the single file `ROVER_PROJECT_FILE` names, and uses it as the default for `rover acquire`'s `--project` and the MCP `acquire_device` tool's `project` argument (D22). Nothing else in the file is read on that side, no client runs what one declares, and the wire is unchanged — the lease still carries `project` as the required opaque string it always was, so this is about who types it and nothing more. `owner` is never defaulted, from this or from anything (D16, D20). Because the MCP declaration is what the SDK validates a call against, the argument goes optional in the declaration itself and only when a file is configured, with the optional variant derived from `AcquireDeviceParamsSchema` rather than written out again. A configured file that is missing or will not parse fails loudly on the client, naming the path, before a device is asked for.

**How the install command reaches the device.** Through a seam of the same shape, from the other end of a lease. `install_app` with no `packageBase64` runs the lease's project's `install` on the host: `src/verbs/files.ts` declares a `ProjectInstaller` — the verb layer names the shape and never the command, because a spawn under `src/verbs/` would be a spawn in every client's module graph — and `src/daemon/project-install.ts` fills it from the hook file through the same `hook-command.ts` runner, with `ROVER_DEVICE_SERIAL` set to the leased device so an install can never land on a neighbour's. It is a verb a caller asks for and never something that happens at grant time. Every way it can go wrong is a **named** verb failure rather than `internal_error` — no hook file, no `install` in it, or a command that ran and exited non-zero (carrying the code, the signal and a stderr tail) — because each is a different next move for the agent. Its bound is `INSTALL_HOOK_TIMEOUT_MS` (five minutes), which lives in the verb layer for `FRAME_EXTRACTION_TIMEOUT_MS`'s reason: it is generous because a build is not a teardown, it sits a long way under the twenty-minute lease TTL that is renewed when the call arrives (D8), and it is well past a client's 30 s default request timeout, which a caller asking for one has to raise itself. **And it is cancelled with the lease.** An install is the one thing a verb awaits that revoking a backend cannot stop — a revoked backend refuses the *next* method call, and a spawned process makes none — so `VerbTraffic` carries an abort signal beside the guard, `verb-handlers.ts` hands it down to the install, and a release or an expiry kills the build. Without that the five minutes would not be this caller's wait but the device's: the ending lease's restoration waits for its verb calls to unwind and every `acquire_device` waits on the restoration, so the next agent would sit behind a build nobody is waiting for any more. The restorer bounds that wait at `SETTLE_TIMEOUT_MS` regardless, the way it already bounds the teardown — a bound on the wait, never on what it is waiting for.

**How the helper services come up and go down.** Two halves, on the two ends of a lease, and neither is a caller's to ask for. `src/daemon/project-services.ts` starts them inside `acquire_device` — **after** the device re-verification and the previous lessee's restoration, and **before** the grant is answered — so a caller holding a lease has the services that lease implies. A start that fails **refuses the grant by name** through the existing refusal vocabulary (`service-failed`): granting a device whose helper services are down is a false yes, so everything that grant had already brought up is stopped again, the lease it took is released, and the device is left free for the next caller. It is the one awaited step below the store's insert, which is safe in both directions — exclusivity is already settled by then, and the start never throws, so the "nothing that can throw after a successful acquire" rule holds. A hook file that will not parse refuses a grant too: a file the host cannot read is a file whose services it cannot start. The whole phase is bounded by `SERVICE_START_TIMEOUT_MS`, under a client's 30 s default request timeout, because `acquire_device` is the one call no client raises that for — which is also why a start must be a *start*, with readiness probes, health checks and restart-on-crash all deliberately absent. That bound covers the **refused grant's rollback** as well as the starts: it is inside the wait the caller is doing, and a rollback that ran past it would answer `service-failed` after the client's timeout had already turned it into an opaque host error with no service named. A stop the budget runs out before is warned about rather than run.

The **stopping** half is the restoration's, ahead of the project's own teardown and in the reverse of declaration order, each stop contained and bounded like the teardown (D9). Every stop runs, for every lease that ends — and what makes that correct rather than a teardown reaching into a neighbour's state is R18, which landed first. Each lease has a slot, every hook child is told its index and its port block, and both halves here go through the same `runHookCommand`: a `start`/`stop` pair that namespaces on `ROVER_SLOT` brings up one instance per lease and the restoration takes down **that lease's** own, which is what lets two devices be leased for one project at once. A pair that ignores the slot addresses a single shared instance instead, and then the first lease out takes it away from whoever else holds the project. That contract is the hook file's to keep and cannot be moved into the host: a hook is an opaque command, and nothing can read one and tell which shape it is — so it is stated in `docs/MANUAL.md`, where the file is written. Reference-counting the leases on a project instead was considered and rejected: it would fix the shared-instance shape by breaking the slot-scoped one, leaving the first lease's own instance running until the last lease ended and then stopping only the last one's — a silent leak in place of a documented contract. What a grant started is tracked per lease and dropped on the lease store's end hook, the way `ArtifactArchive` drops its sequence counters, so the host does not grow with the leases it has granted; that record deliberately decides nothing about what gets stopped, because the hook file is re-read at the lease's end and caching it in host state for a whole lease is what D6 says not to do.

**How the teardown reaches the device.** Through the restorer's existing seam and nowhere else. `src/daemon/restore.ts` names a `ProjectResolver`; `src/daemon/project-resolver.ts` fills it from the hook file, and `src/daemon/hook-command.ts` — the only one of the three that starts a process — runs the declared command bounded, with `shell: false`, output tail-limited, and `ROVER_PROJECT` and `ROVER_DEVICE_SERIAL` in the child's environment. That bound is on the hook's own process — the run ends when the child exits, not when its pipes close — so a teardown that backgrounds a helper is finished the moment it exits, and the daemon is never left waiting on descriptors a grandchild inherited. The split is load-bearing: `restore.ts` is imported by `lease-handlers.ts` and must stay free of a spawn (`tests/unit/daemon/remote-never-spawns.test.ts`), and reading a hook file must stay importable from anywhere. Because the seam hangs off the lease store's end hook, the teardown runs on **release and on expiry alike** (D9) — the second being the path where there is no caller left to ask. The same resolver supplies the service stops that run ahead of it, so one read of the file answers both.
**What a hook is told about its lease's ports.** Three more variables beside `ROVER_PROJECT` and `ROVER_DEVICE_SERIAL`, set by `src/daemon/hook-command.ts` for **every** hook child: `ROVER_SLOT` (the 0-based index), `ROVER_PORT_BASE` (the first port of this lease's block) and `ROVER_PORT_COUNT` (how many consecutive ports are its). They are written last, after the hook's own declared `env`, so nothing a project declares can override the one guarantee the daemon has to keep — that no two live leases were told the same numbers. `HookCommandContext.slot` is **required**, not optional: every hook run belongs to a lease and every lease has a slot, so a future third call site cannot quietly run a child that was told nothing. Rover reserves the numbers and never binds or probes them — the project's service is what listens — so the guarantee is bounded honestly, and the constants (`SLOT_PORT_BASE`, `PORTS_PER_SLOT`, `SLOT_COUNT`) are test seams rather than operator configuration. The slot travels through the seams that already exist: `ProjectResolver` takes it beside the serial, and so does `ProjectInstall`. Until the `services` field of the hook file lands, "helper services" means whatever a project's own `install` and `teardown` start.

The `services` field is now part of the hook file: its `start` and optional `stop` commands use
the same slot-aware runner, so concurrent leases receive separate port blocks.
