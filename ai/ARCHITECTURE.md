...
**What it carries today.** `project` (which must equal the file's own name), `apps` — the applications a lease on this project drove — and two commands, `install` and `teardown`, each declared as a program and its arguments rather than a shell line. Both are optional and neither is defaulted: a default here would be the core naming an application, which is the bug D13 exists to rule out. The helper services are named in D13 and are not in the schema yet: a field lands with its consumer (`ai/RULES.md` §7). `src/daemon/project-hooks.ts` is the Zod schema and the reader; `README.md`'s configuration section mirrors it.
