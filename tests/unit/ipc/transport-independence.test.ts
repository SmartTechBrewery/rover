 ...
 *
 * D17 makes the network listener (R22) an added transport rather than a rewrite, and that
 * only holds while the message surface cannot tell what it is speaking over. The way it
 ...
 */
const FORBIDDEN_MODULES = ['node:net', 'node:fs', 'node:child_process', 'node:os', 'node:tls'];
