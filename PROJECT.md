...
| `release_device` | Hands it back and restores the original state — the applications, the radios, the project's helper services, then its teardown hook (D9) |
...
the MCP adapter can silently diverge. The file-transfer tools remain outside MCP; R24 phase 2
delivered their CLI commands.
