...
passing backend from a check that stopped checking. R46 and R47 are what remain, and they were the
two capabilities that manifest declares **`false`** and can honestly flip later — **one of which
has flipped since** (#251, R47 phase 4): `canReadScreen` is `true`, answered by idb's accessibility
read over gRPC, leaving `canInput` as the last `false` on this manifest that is not a permanent
opt-out.
