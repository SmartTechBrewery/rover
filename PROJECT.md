...
- The MCP server was driven over stdio rather than through an MCP client's own configuration: three
  frames in, the handshake and **22 tools** back, plus a real `list_devices` call against the
  device and both startup guards (an incomplete `ROVER_HOST_*` set, and a `ROVER_PROJECT_FILE`
  naming no file).
