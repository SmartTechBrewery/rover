...
| R18 | Per-slot helper service port allocation | No race, with recovery after an orphaned slot. The precondition for parallel work with more than two devices. R17 phase 4 starts and stops what a project declares and allocates **nothing**, so until this lands two concurrent leases on one project share whatever ports that project's services hard-code — this row is what makes two devices' services able to run side by side | R17 | S |
