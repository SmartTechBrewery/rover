...
- **`--time-limit` still bounds a recorder whose adb client is long gone.** A detached recorder
  ...
  phase 3's lease-end teardown lands, and it is why it is always passed and never `0`.
...
- **`group_id` is a fifth opaque string, is *not* part of the tree's shape either, and is the one
  that spans leases** (D22, as amended #150). Several leases carrying one group id are one
  investigation. It cannot be a directory name — it would be a *fifth level*, and the tree is
