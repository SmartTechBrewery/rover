...
| R29 | `list_devices` carries when a lease was granted | `LeaseHolderSchema` gains the grant instant alongside `expiresInMs`. A client can render `GRANTED` without inference: the two are independent, because activity renews the expiry (D8) and never moves the grant. **No `leaseId` joins the listing** — D20's reason for its absence is unchanged. A unit test covers a renewed lease: expiry moves, grant time does not (#107) | R8 | XS |
