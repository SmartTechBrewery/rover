...
- **Retention is undecided** (§9.4) — without one, this grows without bound on machines that
  ...
  reading this archive on a schedule (D24). **The `Keep` flag above does not decide it and must not
  be read as deciding it**: it records which tests an operator wants exempted from a sweep that
  does not exist yet. No TTL, no size cap, nobody nominated to run the prune, and nothing in this
  repository deletes an artifact today. What the flag does buy is the order — the sweep now cannot
  ship ahead of the exemption and delete a test somebody had every reason to believe was kept.
