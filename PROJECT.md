...
- **`group_id` is a fifth opaque string, is *not* part of the tree's shape either, and is the one
  ...
  `list_archive_groups`** (#178): the host's third read of this archive answers which groups exist,
...
- **A `label` is an artifact's own name and it lives in the *file name*, not in the tree** (D22, as
  ...
  **It is read back out of the name by `list_archive_groups`** (#178), and what comes back is the
  ...
  is the same thing at two moments* needs. A **sidecar file recording the caller's exact label** was
