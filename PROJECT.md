...
| D37 | **The disk budget is enforced after every lease ends — released and expired alike — and the age limit is not** | A run finishing is the only moment this archive can *newly* cross its size: nothing else on the host writes into the tree, so a budget checked at any other moment is checked when nothing has changed. It therefore hangs off the path D9 already runs, which is what makes it a teardown rather than a happy path — an expiry restores a device with no caller left to ask, and it sweeps for the same reason. It is deliberately **behind** the release rather than inside it: `release_device` answers as soon as the store has forgotten the lease, the restoration is queued after that and the sweep after *that*, so nothing an agent is waiting on ever waits for a walk of the archive — and a sweep that fails leaves the release successful, with one line on the host's own log saying the archive is a sweep behind. The **one** thing that does wait for the walk is a **shutdown**, and it has to: the unit of deletion is a whole run directory, so a `process.exit` landing inside that `rm` would leave a run every listing still reports while it holds a subset of what its lease wrote — and a partial deletion that happened to bring the tree under budget is never selected again. So `close()` settles the sweep in flight beside the restorations it owes, bounded and warning at the bound for D6's reason: a `close()` that never resolves is worse than a husk reported out loud. The **age** bound is deliberately left out of this trigger: a lease ending makes nothing older, so enforcing it here would be a rule fired by an event that cannot change its answer. It needs a clock, and it has one now — D38, which is that own change. The accepted cost is one walk of the whole tree per lease, and it was **measured rather than assumed** (§6): ~150 ms for a full archive at the default 1 GiB budget and ~610 ms for one four times over it, linear in the file count and off the answer's path in both cases. So **no cached total was built** — the measurement does not call for one, and D6 binds it in any case: a total is re-derived, never trusted from disk | 2026-09-08 |
| D38 | **The whole policy runs on a clock: one full pass at local midnight and one at daemon start, paced by a comparison against when the pass last ran rather than by the timer** | The *age* bound had no unattended trigger at all — a lease ending makes nothing older (D37), so an archive sitting comfortably inside its budget could hold runs from months ago until an operator typed `rover sweep`. A clock is what that bound needs, and it needs **two** faces of one. Midnight is the pass; **start** is what covers the host that was not running at midnight, which on the machine this is built for is most nights — a developer's Mac with phones on it is asleep, and a machine that was switched off has no timer to be late. The pass therefore **does not trust the timer**: before sweeping it asks whether the last pass was before the local midnight now behind it, and re-arms without sweeping when it was not. That comparison is the whole design, and it is what makes *does a Node timer survive macOS sleep* a question nobody has to answer — deliberately **not measured**, because either answer lands on a path already handled. It is also what keeps **DST** to exactly one pass a day in both directions: arming is always from *now* to the next local midnight through `Date`'s own local accessors, and the guard is a **local** day rather than 24 hours, because the day a zone springs forward is 23 hours long and a flat comparison would skip it while the day it falls back holds `00:00` twice. The last-run stamp is **in memory and not persisted** (D6): a stamp on disk would be state the daemon cannot re-derive, and the start pass is what covers a restart. The schedule is its own module (`src/daemon/retention-schedule.ts`) rather than a timer inside the sweeper, so what a pass does and when a pass happens stay separable and the clock arithmetic is a unit test rather than a suite that waits for midnight; it is started only by the **winner of the bind** and stopped first on the way out, exactly as the lease-sweep interval is, because two daemons deleting out of one archive root would each be answering about a tree the other had already altered. Two costs were stated and accepted: the disk may stand **over budget for up to a day** in the worst case a midnight pass covers, and a **wall-clock hour** enters a system that otherwise keeps none — no `sleep` anywhere, and a timer handed a callback that does something is a deadline rather than a delay awaited instead of a check (ai/RULES.md §2, D12(b)), so this needs no exemption from the no-sleep gate | 2026-09-08 |
| D39 | **Rover installs its own second-order tooling on the host, into `~/.rover`, and the search looks there last — but never a platform SDK, and never as a side effect of `npm install`** | `idb_companion` is the first program Rover needs that has no way of arriving: brew dropped it with the `facebook/fb` tap, there is no installer, and the supported route was *unpack a release tarball wherever you like, then tell Rover where that was*. Measured on a real setup on 2026-09-09, that is three ways to get it wrong and all three are silent. `~/.local/bin` is on no stock macOS `PATH` (it is in `/etc/paths` on no mac; a user's own rc file is what puts it there), `/usr/local/bin` **is** on that `PATH` but needs `sudo`, and `ROVER_IDB_COMPANION_PATH` is read by the **daemon**, which autostarts (D5) inheriting the environment of whichever client woke it — a shell, a panel, or an agent's MCP server with its own `env` block, so exporting it in the terminal you are typing in fixes nothing when something else got there first. The one place both halves already agree on is the host's own `~/.rover`, beside the socket, the users file and the archive. So `rover doctor --fix` unpacks a **pinned, checksummed** release into `~/.rover/idb-companion-<version>/`, and that directory is the search's **third row** — which reverses this repository's own “there is deliberately no third row, because this program has no canonical install location at all” (R47 phase 1, edited in place). That premise was true exactly as long as the *operator* was the one unpacking; the day Rover unpacks, a canonical location exists because Rover chose it. **Last of the three rows**, so a companion the operator installed themselves always wins: theirs is a decision, Rover's is a default, and a default that shadowed a choice would swap the binary under somebody who had pointed the variable at a build of their own. **Never a platform SDK.** Rover fetches the second-order tooling nothing else ships — never `adb`, which arrives with platform-tools and whose stray copy would kill another tool's adb server on a version mismatch (D32), and never Xcode. Those are reported and left alone, which is what `installable: false` on a row means. **Never from `npm install`.** A postinstall that downloaded 19.5 MB from GitHub is the side effect that makes a package untrustworthy, and it would do it inside every fresh worktree an agent creates; what `npm install` does is *warn*, naming the one command (`scripts/check-idb.mjs`, `./check-adb.mjs`'s report-and-change-nothing rule). The install is an operator asking by name, with `--actor` attached and an audit line written (D28). **On the host, through IPC, not in the CLI** (D19): the program has to be on the machine the devices are on, which is not necessarily the laptop the command is typed on, and a client cannot reach a backend at all (`tests/unit/no-backend-in-a-client.test.ts`). Which programs exist and which can be fetched is each backend's own declaration on its registration record (`hostTooling`, beside `stopHostProcesses`), so a third backend's tooling appears in `rover doctor` with no shared code edited (ai/RULES.md §2). **`list_host_tooling` is the one answer on the surface that carries host paths on purpose**, against the habit `list_archive` and the kept-tests handlers keep: there the path is incidental, here *which* copy won the search **is** the diagnosis. Neither row is an MCP tool — an agent that meets an unbacked capability already gets a `missing-capability` failure naming the program, which is a sentence to relay to a person rather than an install to attempt | 2026-09-09 |
| D40 | **Starting a host and serving the panel are `rover` commands, not npm scripts — and the foreground start is the only one that keeps a host reachable** | `npm run daemon` and `npm run panel:dev` work only from inside this checkout, which is the gap `npm link` already exists to close for `rover init`: an operator whose devices are on this machine is usually sitting in *their* project, where `npm run` runs that project's `package.json`. That alone would only be convenience. What makes it a decision is what the autostart deliberately does **not** do: `src/daemon/connect.ts` clears `ROVER_LISTEN_PORT` and `ROVER_HTTP_PORT` in every daemon a client brings up, because a host that began listening for other machines or for browsers as a side effect of `rover list` would be a change in exposure nobody chose (D17, D29). So the *only* way to run a reachable host was an npm script in one directory, and `rover server` is that start made first-class: the caller's environment passes through untouched, `stdio` is inherited so the host's own warnings reach the terminal that asked for them, and the two signals are forwarded so the shutdown that releases leases actually runs. It takes **no `--host`**, with `users`, `init` and `doctor`'s plain form: it does not talk to a host, it becomes one, and always this machine's. **Spawned, never imported** — the daemon reaches every backend and a client that imported it would be a client that can drive a device (D19), so the entry comes from `connect.ts`, which already owns where it is, and the spawning from `src/cli/_shared/foreground.ts`. That module is the **second** place a client may start a process, beside the autostart, and both spawn gates name it explicitly rather than exempting a directory. `rover panel` is the same shape for the page, and it is honest about being Vite: the daemon answers `/rpc`, `/session` and `/artifact/…` and does not serve `panel/dist`, so the page still comes from this repository's development server and the command needs its `devDependencies`. Its usage text says so, and says what it becomes when the host serves those files — a URL rather than a server. **The port is `ROVER_HTTP_PORT` in both halves**, the daemon's own switch that `panel/vite.config.ts` already reads, so there is one number to keep in step and not two. The word is `server` rather than `daemon` at the operator's request; everything inside the repository still calls it the daemon, and the command's own help binds the two so nobody reads them as different things | 2026-09-09 |
...
passing backend from a check that stopped checking. R46 and R47 were what remained, and they were
the two capabilities that manifest declared **`false`** and could honestly flip later — and
**both have flipped since**: `canReadScreen` (#251, R47 phase 4), answered by idb's accessibility
read over gRPC, and `canInput` (#252, R46 and R47 phase 5 together), answered by one
client-streaming `hid` call behind `tap`, `swipe`, `typeText` and `pressKey`, with `back` and
`recents` refused **by name** rather than substituted for. `canControlNetwork` is the only `false`
left on that manifest, and it is the permanent opt-out.
...
### A daily pass measured in hours skips a day once a year (2026-09-08, #246)

- **A spring-forward local day is twenty-three hours, so `now - lastRun >= 86_400_000` refuses the
  pass that day.** This was the shape the plan for D38's clock trigger had been written with, and
  it is wrong in a way only the arithmetic shows. In `America/New_York`, `2026-03-08 00:00` to
  `2026-03-09 00:00` spans 23 hours: the shift is at 02:00 on the 8th, so the *following* midnight
  arrives an hour early in absolute terms. A pass armed for it therefore fires with 23 hours behind
  it, a flat day-length comparison declines, and the schedule re-arms for the midnight after that —
  one skipped day per year, in the *none* half of the failure the row exists to avoid. The fix is
  that the comparison is a **local day** rather than a duration: sweep when the next local midnight
  *after the last run* has arrived, which is the same `nextLocalMidnightMs` the arming uses. The
  fall-back direction then needs no second case — a 25-hour day is one midnight later just as a
  23-hour day is, and a wall-clock `00:00` that happens twice is still one local date.
- **A timer's own accuracy is not what to fix here.** `setTimeout` counts on a monotonic clock while
  this comparison reads a wall clock the host may adjust under it, so a fire can arrive a moment
  *before* its target. Rather than a tolerance, a fire that is not yet due re-arms for what is left
  of the gap — an early fire then costs one extra timer instead of a skipped day, and no slack
  constant has to be guessed at.
...
| R46 | The per-key refusal this platform needs before `canInput` can be declared | Shared code carries the per-argument refusal already (#215), so what is left is this platform's answer to the key vocabulary `DeviceKey` fixes: `back` and `home` are answered, `wake` reads lock state first so the press is idempotent, and the key with no equivalent is refused **by name** with an `unsupported-key` failure rather than answered with something that is not the key asked for. Its own row because the decision is about the shared vocabulary rather than about one backend's plumbing (`docs/IOS.md` §5, §10 step 3). **Done** (#252), delivered with R47's fifth phase because `CAPABILITY_METHODS.canInput` names `pressKey` beside the other three and the conformance gate will not take a manifest that declares the capability with any of them missing — and **two of the three answers above are corrected in place with their reasons rewritten** (ai/RULES.md §1), because both were measured for the first time here rather than argued. **`back` is refused, not answered**: `docs/IOS.md` §5's table had the left-edge swipe "verified working" off one drill into Settings, and driven through this backend the same `2,450 → 300,450` swipe **paged the home screen** on Springboard and did **nothing** on a modal sheet, both at exit 0 — a gesture that is silent in one direction and some other navigation in the other is precisely the substitute `pressKey` must not make, and on iOS back is a control in the app's own UI that `read_screen` + `tap` already reaches by label. **`wake` reads the *blanked-screen* flag rather than lock state**: `LOCK` does not unlock — a locked, woken simulator stays locked however many times it is pressed — so lock state is the wrong question, and the right one is the one Android's `KEYCODE_WAKEUP` also answers. `com.apple.springboard.hasBlankedScreen`, read with `simctl spawn <udid> notifyutil -g <name>` in ~360 ms, **is** readable, which is what made this a whole answer rather than the refusal this row's fallback allowed for: measured on the bench (iPhone 17 / iOS 26.4 / companion v1.5.2, 2026-09-09) the flag followed the press in 1,861 ms going dark and 347 ms coming back, every unguarded press toggled it, and three guarded `wake`s in a row left it at `0`. That read cannot prove the *name* still exists — `notifyutil -g` answers `<name> 0` for a name nobody publishes, indistinguishable from a woken device — so `tests/device/ios-simulator/input.test.ts` drives the flag through both values against the runtime in front of it and fails if it stops moving. `recents` is refused as accepted on 2026-09-08, and the two refusals over a lease are this repository's first per-key refusals that come from a device rather than a synthetic backend | R45 | S |
| R47 | idb — the screen read, the four input primitives, and a real device stream | `docs/IOS.md` §10 step 2: `readScreen`, `tap`, `swipe`, `typeText`, `pressKey`, and `watchDevices` moved off the `simctl list` poll onto the companion's `--notify` change stream, which emits the full current set on every change and is `DeviceWatcher.onDevices`' contract to the letter (§7). Talk gRPC from Node rather than shelling out to the Python client, supervise one companion per target, **never** call its file push (it kills the companion, §4), and classify a companion crash as an interruption rather than a device fault. Checkable: `canReadScreen` and `canInput` flip to `true` with the conformance suite green, and a second simulator booting produces one full-set update per transition with no polling anywhere. **This row is being delivered in five phases**, the way R44's four and R45's five were: the pure layer under idb, then `watchDevices` on the stream with the poll as fallback, then one supervised companion per target over gRPC, then `readScreen`, then the four input primitives — and `capabilities.ts` moves in the last two, beside the abilities it declares. **Phase 1 is done** (#216): where `idb_companion` is on this host, and what its `--notify` target stream says. **Nothing spawns a process, nothing registers and no capability moves** — the shape `developer-dir.ts` and `parsers/simctl-list.ts` already set for this backend's first external program, and `tests/unit/backends/barrel.test.ts` and `conformance.test.ts` still read `['android', 'ios-simulator']` with the label unchanged at `iOS Simulator (simctl)`. `src/backends/ios-simulator/idb-companion-locations.mjs` is the search and `idb-companion-path.ts` is the TypeScript face over it, and it has **three rows** — this sentence is edited in place with its reasoning rewritten (ai/RULES.md §1), because it said **two rows and no third** and D39 reversed that. The rows are `ROVER_IDB_COMPANION_PATH` taken verbatim (empty counts as unset), then every `PATH` entry in order with an empty entry skipped, then `~/.rover/idb-companion-<version>/` — the copy `rover doctor --fix` unpacks — and off macOS the list is empty so every suite stays runnable on a Linux runner. The two rows were two because this program had **no canonical install location at all**: brew no longer carries it, the old `facebook/fb` tap is gone, and the supported install was an unpacked release tarball wherever the operator put it, which is the argument README makes for `ffmpeg`, with the override added on top because a tarball in a scratch directory is on no `PATH`, least of all the short one a GUI-launched daemon inherits (D5). That premise held while the **operator** did the unpacking and stopped holding the day Rover did it: the third row is where Rover puts it, it is **last** so an operator's own install always wins, and it is why the failure now ends by naming one command instead of a tarball, a directory choice and a variable. Never a walk of the disk (D32). **A candidate is accepted on the filesystem rather than by being run**, which is one step short of `adb-path.ts` and deliberately so: `adb version` is acceptable only because it was *measured* to start no server, whereas one companion per target is something this backend has to supervise rather than start as a side effect of looking — and nothing spawning a process is what lets the suites import the module on a machine that has never had idb. Phase 3 starts a companion on purpose and is where a side-effect-free flag could be measured; what was measured here is that `--version` prints only `{"build_date":"Sep 1 2026","build_time":"08:51:20"}`, so a capture is pinned to the **release tag** it was downloaded from and not to anything the program will say. Unmemoised, `developer-dir.ts`' stance, since a companion arriving on a running host is what phase 2's restart-on-a-backoff exists to pick up. `IDB_COMPANION_MISSING` is the `tooling-missing` cause naming `idb_companion` and it needed **no shared-code edit**: `InterruptionCauseSchema.tool` is already a free program name, so a backend's *second* program asks nothing of the device interface. `parsers/idb-notify.ts` is the frame decoder and the vendor schema, both written from a capture taken on this repository's own bench — companion **v1.5.2** on Xcode 26.6 / iOS 26.5, 2026-09-08, `idb-notify.idbcompanion1.5.2-xcode26.6-ios26.5.txt`, the companion's version leading the filename because the companion is what governs this format. The framing is **one JSON array per line, newline-terminated**, each line the full set; the decoder holds the trailing partial and a frame it cannot read is terminal for that run (`TrackFramingError`'s reasoning — framing that has lost sync cannot be resynchronised, and a device list assembled from a guess is worse than none). **The terminating newline arrives in its own write** — the reads came in at `1784, 1, 1784, 1783, 1789, 1, 1785` bytes — which is why this is a decoder rather than a `split('\n')` per chunk. The target schema is a non-`.strict()` projection of Meta's six keys with `state` **and** `type` open strings, for `SimctlDeviceSchema`'s reason on the first and because this bench had no physical target paired on the second: an enum there would refuse the one frame the exclusion rule exists for. `toNotifiedDevices` is a second entry point in `devices.ts` rather than a module of its own, so the platform id, the `Booted`-is-the-only-`ready` predicate and `SIMULATOR_ATTACHMENT` are each decided once — `toDeviceState` now takes a bare state token so both paths share it literally. Two decisions there are not mechanical. **`osVersion` is normalised to the version alone**: idb reports `iOS 26.5` where `simctl`'s runtime reports `26.5` for that same runtime, and two spellings for one device would let the watch and `listDevices` disclose two different OS versions — the disagreement `deviceInfo`'s `model` note already refuses on the model field. A second `simctl list -j` capture was taken minutes later on the same bench for exactly that assertion, so the suite compares `toDevices` and `toNotifiedDevices` **device by device** over the same eleven simulators rather than each against a literal, which is the drift that matters. **And only a simulator under an iOS runtime is admitted, as an allowlist**: idb enumerates physical targets, `attachment.ts` records that a paired-but-absent iPhone is served by this platform's tooling by default forever, and a physical device cannot answer `screenshot` — a *required* method (`docs/IOS.md` §6). An allowlist because the direction of the mistake is not symmetric: an unrecognised target excluded is a device this host declines to lend, while one admitted by default is D18's two-agents-one-device failure wearing a disguise. Neither exclusion is in the capture, because no iPhone was paired to the bench and only the iOS runtime was installed, so both are pinned by inline cases named as inline. **What a green suite here cannot claim**, and the change says so: that the parser matches what the companion prints in general. It matches one capture from one version on one host; the first evidence past that is phase 2's device test. **Phase 2 is done** (#249): `watchDevices` runs on the notify stream, with the `simctl list` poll kept as the **fallback**. `src/backends/ios-simulator/idb-companion.ts` is the streaming runner — `spawn`, `stdin: 'ignore'`, `close` rather than `exit`, `onStdout`/`onStderr`/`onEnd(reason)` with `onEnd` exactly once, a `stop()` that kills, and **deliberately no timeout** for `streamSimctlOnDevice`'s stated reason: the one call that uses it is supposed to stay open, so a timeout would guarantee the failure it exists to prevent. The argv is the caller's and the process is the module's, so phase 3 reuses the spawn half for a companion started with `--udid` without a second copy of any of it; `resolveIdbCompanion()` throwing is let out **synchronously** so the watch can classify it, exactly as that runner lets `SimctlNotFoundError` out, and `streamOutcome` is now exported from `simctl.ts` rather than copied a third time. stdout is handed over as **bytes** because a chunk boundary can fall inside a multi-byte character; stderr is decoded, and the **tail** goes into the interruption message rather than into the runner's `reason`, which is that runner's rule that whoever puts a stream into a message is who bounds it. In `watchDevices` the two sources sit behind one delivery rule and **only one delivers at a time**: a frame is the only evidence the stream is healthy, so a frame is what takes the watch off the poll, and while the stream is up `simctl list` is not run at all. **A lost view is one `onInterrupted`, never an empty set** — an empty set reads as every device having gone away, which for an inventory means releasing devices that never moved — carrying `IDB_COMPANION_MISSING` when there is no companion to run, `SIMCTL_MISSING` when the fallback has no `simctl`, and `null` otherwise, because `null` is the claim "this is expected to clear". **The view is whichever source is serving the caller**, which is what makes the fallback exactly one interruption rather than one per attempt: once the poll is delivering, a companion that fails to start again changes nothing the caller can observe and is silent. Restarted on a doubling backoff, `AndroidDeviceBackend`'s `TRACK_RESTART_MIN/MAX_DELAY_MS` shape at 250 ms to 5 s, reset by a frame — a `setTimeout` whose callback does the next attempt and never a sleep — because an `idb_companion` unpacked onto a *running* host is what the restart exists to pick up, the search being unmemoised. A frame the decoder cannot read is terminal for that run and reported rather than thrown, since there is nothing above a stdout handler to catch it. `stop()` silences every handler synchronously, kills whichever companion is live and clears whichever timer is pending. **No capability, method, manifest or gRPC work**: `listDevices`, `describeDevice` and `deviceInfo` keep reading `simctl`, because idb is not a second source of truth for an enumeration a lease grant re-verifies (D6) — what makes the two safe to mix is phase 1's normalisation of the notify path's `os_version` onto `simctl`'s spelling. `tests/device/setup.ts` gains a fifth flag, `ROVER_TEST_IDB`, probed through `resolveIdbCompanion()` rather than a bare name (#171) and **spawning nothing at all**, which is not restraint but the whole of what that search does; it is warned about as loudly as the ffmpeg gate. `idb-companion.ts` is added to `remote-never-spawns.test.ts`' `ALLOWED_TO_SPAWN` deliberately, and it is the one entry there whose child is unbounded by design. **What a green unit suite here cannot claim** is that a real companion behaves as its mock does; the device case that closes that gap asserts the first delivery names the booted device `listDevices` names with the same `osVersion` string, and it skips on a host with no companion. **One trap was hit building this and it would have made the whole stream silently unavailable** (`docs/IOS.md` §8 trap 15): `idb_companion` resolves Xcode through `xcode-select`, so on a bench whose selection is CommandLineTools — the state §1 of that document says a developer machine is most likely to be in — it exits **0 with an empty stdout** and one line on stderr, while `simctl` runs perfectly through `developer-dir.ts`' search of the same machine. So the runner exports `DEVELOPER_DIR` for the child, set to `resolveDeveloperDir()`, which is #171's "never measure two machines" one program further out and preserves an operator's own value by construction since that variable is the first row of the search; a resolution that fails leaves the environment untouched, because the companion's own complaint about a host with no Xcode is more accurate than a substitute. **Driven end to end on this bench afterwards** with one `iPhone 17 Pro` booted and shut down again through the fixture README's own recipe: three deliveries, each the full set of eleven, at +0/+326/+3754 ms and zero interruptions, with no `simctl list` run at all — and the `Booting` frame collapsed rather than missing, since `Booting` and `Shutdown` both map to `offline`, which is the delivery rule saying that the companion emits per change *it* sees while the watch delivers per change a caller can see. **Phase 3 is done** (#250): one supervised companion per target, spoken to over gRPC from Node. `package.json` gains `@grpc/grpc-js` and `@grpc/proto-loader`; `src/backends/ios-simulator/idb/idb.proto` is idb's **own** service definition vendored unmodified at **v1.5.2**, with a header naming the upstream file, the release and its MIT notice, loaded at run time because this repository runs from source through `tsx` and has no build step — generated clients would have to be committed and hand-kept in step with the proto they came from. `src/backends/ios-simulator/idb-client.ts` is the supervisor and the channel, and it **reuses `idb-companion.ts`' spawn half with a `--udid` argv** rather than duplicating any of the process handling, which is what that runner's "the argv is the caller's" was written for. **How the companion is reached was measured rather than assumed, and the answer is a unix domain socket rather than a port** (`docs/IOS.md` §4 carries the table). Both routes work and both are self-reporting: `--grpc-port 0` prints `{"grpc_port":56927,…}` on stdout and `--grpc-domain-sock <path>` prints `{"grpc_path":"…"}`, so nothing has to pre-bind a free port on this host to learn which one was taken. The socket wins on what the port route *binds*: `Swift server started on [IPv6]::/:::56927` is **every interface with no authentication**, and since nothing arbitrates in this stack the lease layer is the only lock there is — an open port carrying the full companion API is a way round it for anyone who can reach a host whose whole shape is being reachable over the network (D17). One `mkdtemp` directory per companion holds the socket and goes with it, so a socket left by a companion this host did not shut down cleanly is never where the next one wants to bind — through **one memoised removal both a death and a `stop()` reach**, handed to the pool so that `stopAll()` resolves on a host that is tidy rather than one that is nearly tidy. A fire-and-forget `rm` was the first shape of that and it was observable from nowhere, which made the case asserting it a race with its own subject. **A start resolves only once the companion is answering**, as two conditions with one shared deadline through `src/core/wait.ts` and never a sleep: the handshake line, then a real `describe`. The second is not redundant on the first — a bound socket is not an answered call, and the probe also warms the lazily connected channel so the caller's own first call does not pay the connect. Measured on this bench, v1.5.2 against a booted `iPhone 17`, 2026-09-08: the socket at **+353 ms**, `describe` answered at **+423 ms**, and a second `describe` on the established channel at **1 ms** against the ~70 ms every CLI measurement in `docs/IOS.md` paid. `IDB_COMPANION_START_TIMEOUT_MS` is 10 s, `RECORDING_START_TIMEOUT_MS`' value at a twenty-fold margin, and every call carries a gRPC deadline because a companion that is wedged rather than dead must not hold a lease open — the handshake's being the **smaller** of that bound and what is left of the start, since `waitForCondition` runs a probe to completion before it looks at its own deadline and an unclamped one would let a companion that binds and never answers cost a start budget plus a whole call timeout. **A companion's death is an interruption, not a device fault.** `IdbCompanionInterruptedError` names the program, the udid and the RPC that was in flight, says the device itself was not disturbed and says another companion is started for the next call; it is deliberately not a `DeviceVanishedError` and never makes a device look offline, because killing one leaves the simulator `Booted` — which this phase re-ran through this repository's own code rather than quoting. **One trap, and it would have made every crash unreadable**: gRPC sees the socket close *before* Node reports the child's `close`, so without help every mid-call death surfaced as `14 UNAVAILABLE: Connection dropped` — a sentence about a transport. So `UNAVAILABLE`, and only `UNAVAILABLE`, is given a short bounded grace to be explained by the process ending; a status the companion *chose* is its answer and is passed straight on, and a grace that runs out with the process still running stays the plain failure rather than promising a replacement nothing will start. **Restart-on-death is the next call's job.** A dead companion is forgotten and the call after it starts a fresh one — there is no background respawn, no health timer and no task of this module's own, because every entry point here is reached from a backend method behind a lease and a supervisor with a schedule of its own would be the thing that reaches a device outside one. A start that failed is forgotten the same way, `idb-companion-path.ts`' unmemoised stance, so a host that gains a companion is picked up. **`push` is never called, and the rule has an executable half**: `IDB_RPCS` is the closed list the call surface is typed from — one entry, `describe`, because one is what this phase calls — and `tests/unit/backends/ios-simulator/no-idb-file-push.test.ts` reads the RPC's name out of the vendored proto so a rename cannot make the gate pass silently, checks it is off that list, checks no file that can reach a companion names it as a string, and pins that **exactly one module in `src/` may import `@grpc/*`** — which is what makes the scan precise rather than a grep for a word that is also `Array.prototype.push`, `adb push` and this repository's own `push` verb. #228's host-path `pushFile` stands untouched. **No capability, method or manifest change, and nothing dispatches to it yet**: `readScreen` (phase 4) and the four input primitives (phase 5) are what will call this, and `watchDevices` stays on the notify stream, which is a stdout pipe on a host-wide companion and not a gRPC call. `idb-client.ts` needs no `remote-never-spawns.test.ts` entry either, because it imports no `node:child_process` — the process is `idb-companion.ts`', which is already on that list. **What a green unit suite here cannot claim** is that Meta's program behaves as the stub does: the lifecycle is driven against `sh` stubs and a Node program serving the vendored proto in a `mkdtemp` directory, so the suite needs no idb and passes on Linux, and `tests/device/ios-simulator/idb.test.ts` is what closes that gap — gated on `ROVER_TEST_SIMULATOR` **and** `ROVER_TEST_IDB`, it starts a companion for the booted device, asserts the target it describes is the udid it was started with and `Booted`, kills it, and asserts through `describeDevice` that the device is **still `ready`**, then that the next call starts a fresh companion and is answered. The one claim it leaves to the unit suite is what a call in flight sees when its companion dies: `describe` answers in a millisecond against a real companion, so killing one mid-call is a race rather than a case, and the stub that exits 133 without answering — the shape of the file-push crash — is how that is asserted deterministically. **Phase 4 is done** (#251): `readScreen` answers `ScreenElement[]` from idb's accessibility read, and `canReadScreen` is the third `true` in this manifest with the conformance suite green on it — a declared capability with no dispatch is what that gate exists to fail, which is what makes the flip safe. The call is `accessibility_info` in the **`LEGACY`** format over phase 3's channel, and the format is a measured choice rather than a default: all three were captured off one screen. `LEGACY` is **one flat JSON array of nodes**, which is the shape `ScreenElement[]` is and the one `idb ui describe-all` asks for; `NESTED` is the same nodes as a tree, so a consumer flattens one to ask the same question; `COMPLETE` renames every key (`AXLabel` → `label`, `AXUniqueId` → `identifier`), drops `role` and `AXFrame` and wraps the tree in a provenance document, and its own proto comment says an older server silently serves `LEGACY` instead, so a client asking for it has to read both shapes anyway. `ACCESSIBILITY_FORMAT` is exported from `src/backends/ios-simulator/parsers/accessibility.ts` rather than named at the call site, because the format and the schema are one decision. **Three keys of sixteen are projected**, non-`.strict()` for `SimctlDeviceSchema`'s reason — an idb release adding a field must not turn into a failed screen read in a module that reads three keys. `frame`'s four numbers rather than `AXFrame`, which is the same rectangle spelled `{{x, y}, {w, h}}` and would mean parsing Apple's spelling back into numbers the tool had already produced; plus `AXLabel` and `AXValue`, which are **two different strings and are kept apart** — Safari's address field reports `AXLabel: 'Adres'` and `AXValue: 'Szukaj lub podaj witrynę'`, the accessibility name of a control and the text showing in it, and conflating them taps the wrong thing. `frame` is **required** and a node without one fails the whole read, because `ScreenElement.bounds` is not nullable and the only alternative to refusing is a screen read quietly missing an element the device reported. **Pinned against three captures from a real simulator**, companion **v1.5.2** on Xcode 26.4.1 / iOS 26.4.1, 2026-09-08, taken through this repository's own gRPC client rather than through the Python CLI this backend never runs, and each earns its place: a Compose Multiplatform app (`docs/IOS.md` §2's claim in a file — fifteen nodes with `AXValue: null` on every one, so a mapping that hard-coded `null` for the text would pass against that capture alone), Settings › Camera (four `AXCheckBox` nodes whose `AXValue` is the **string** `'0'` or `'1'`, which is what makes `z.string()` a measurement rather than an assumption; one node with no label; a `role_description: ''`, so the empty string is not a defensive case; and a row ending at 883.67 on an 874-point screen), and Safari's start page (the two-different-strings case, and the `AXUniqueId` measurement below). **One sentence of `docs/IOS.md` §2 was wrong and is corrected in place** rather than deleted: `AXUniqueId` is **not** `null` throughout. It is `null` on all fifteen Compose nodes, which is the app that observation was taken on, and on **Apple's own Safari** it is populated on **eleven of eighteen** nodes with `favoritesItemIdentifierContent` on **three** of them — the three favourites tiles. So the field is not merely absent on some apps, it is **not unique within one read** where it is present, and `findOnScreen` treats two elements with one id as the backend contradicting itself (`src/verbs/errors.ts`), which would make Safari's start page unaddressable. The conclusion is unchanged and its reason is now stronger: the synthesised **flat** ordinal is the only truthful id available — flat because this read answers a list where uiautomator answers a tree — carrying `../android/screen.ts`' caveat unchanged, that an id is stable only for as long as the shape of the read is, which is why the verb layer re-reads the screen inside every verb rather than remembering one. `toScreenElements` sits beside `toScreenInfo` in `screen.ts` and **converts no coordinates at all**, which is the one thing that function is worth reading for, because the division the Android function of the same name performs is exactly what a reader will look for here: idb reports frames in **points**, the same space `widthDp`/`heightDp` are in, and the `AXApplication` node of every capture is `{x: 0, y: 0, width: 402, height: 874}` on an iPhone 17 whose profile says 1206×2622 px at scale 3. Unrounded and unclamped for `toScreenInfo`'s own reason, and a rectangle extending past the bottom edge survives because `src/verbs/target.ts` is the layer that decides what is addressable. `''` becomes `null`, because `ScreenElement`'s two text fields are nullable precisely so "carries neither" is representable and `''` would match a substring target for `''`. Every node, in the order the tool listed them, unfiltered — which nodes are interesting is a policy the verb layer already applies by matching on text. **idb's own `density` is still not read**: it reports `3.0`, which is the *scale* where `ScreenInfo.density` wants the DPI (460 on an iPhone 17), so `deviceInfo` keeps taking both numbers from the device type's `profile.plist`. On the backend the method is three steps and no arithmetic — the state check, the call, the mapping — and what is worth stating is what it does **not** do: no conversion, no filtering or sorting, no lock (two overlapping reads of one device share the companion and its channel), and **no app scoping, because the read cannot be scoped** — what comes back is whatever is frontmost, system UI included, so a read immediately after `launchApp` can answer with the *previous* app's tree, which is D12(c) being unavailable on this platform for launches and is why the verb layer's waits are on conditions. The whole-screen request is deliberate in the same way: `AccessibilityInfoRequest` offers three ways to narrow one and none is asked for — `point` is the `describe-point` route and nothing in the verb layer wants a single point (`src/verbs/target.ts` resolves against a whole screen), `marker` is target resolution done by idb instead of by the layer that owns it, and `keys` would be a second place where the key set is decided to save thirteen of sixteen keys off a 40 ms call. **The state check's reason is a process rather than an answer, and this corrects the plan's**: `accessibility_info` against a `Shutdown` device refuses properly — gRPC `INTERNAL` in **13 ms** with a message naming the state — so there is no tool timeout to pre-empt the way a capture's 60.68 s block needs one. What the check buys is that **no companion is started for a device that cannot answer**: reaching that refusal starts one first, and it then stays alive by design, announcing on its own stderr that it "will stay alive if target goes offline", so skipping the check would leave a supervised `idb_companion` per unbootable device anybody asked about for the lifetime of the daemon, to deliver an answer the enumeration in front of it already had. Measured on this bench: a companion starts in **350–423 ms**, the **first** accessibility read of its life takes **3.34 s** because that read is what loads the simulator's `AccessibilityPlatformTranslation`, and every later read on the same companion came back in **34–47 ms** — both two orders of magnitude inside the call timeout, and the first-read cost is stated in the method because it is the one number a caller would mistake for a hang. A companion that dies mid-read fails the call as an **interruption**, never a device fault, phase 3's classification unchanged. `IDB_RPCS` gains its second entry, and `backend.ts` gains **`stopIdbCompanions()`, which is deliberately not on `DeviceBackend`**: that interface has no teardown at all, every method on it being a question about a device. **It is instead an optional `stopHostProcesses` on the *registration* record, and the daemon's shutdown calls it** (`src/backends/manifest.ts`, `stopBackendHostProcesses` in `src/backends/registry.ts`, `BACKEND_STOP_TIMEOUT_MS` in `src/daemon/listen.ts`) — **corrected during review of #257**, where the first shape of this said only the device suite calls it and that a caller would arrive "when the daemon gains a shutdown path". The daemon already had one, and this phase is what makes the gap reachable in production: after phase 3 nothing dispatched to the transport, whereas `readScreen` is now reached both by `read_screen` and by `captureAfterState` after every action verb, so without a caller the count grows to one supervised `idb_companion` per simulator anybody has read, held for the daemon's whole life, with no operator route to releasing them short of killing it. Registered rather than declared on the interface because Android needs none — its `adb` server is not this process's child — and it runs **last** in `closeServer`, after the watches have stopped and the restorations have settled, because a companion is the transport a verb call rides and an earlier kill would be re-created by the very next backend call. Bounded like every other step there (a shutdown that cannot finish is worse than a child reported as left behind, D6) and `allSettled`, so one backend holding on is a warning naming the platform rather than a rejected shutdown. The device suite is still the other caller, and stray companions are not harmless either way, since two on one udid both bind and both accept commands, which is exactly the arbitration the lease layer is the only lock for. **The pool is not invalidated when a device leaves `ready`, and that was measured rather than assumed** (also during #257's review, `docs/IOS.md` §4): on a *throwaway* simulator created and deleted for the run, a companion started for a booted device survives `simctl shutdown` of that target — the read in between is the same well-behaved gRPC `INTERNAL` refusal the state check pre-empts — and answers the next read **correctly over the same channel** once it is booted again, in 163–801 ms and then 24 ms warm. So the reuse is right, an eviction would only buy back the 3.34 s, and this is not a case in the read-only device suite precisely because shutting a simulator down is the one thing that would stop that suite being safe against a device someone else holds. **`canInput` stays `false` and the label stays `iOS Simulator (simctl)`**: naming idb while the input vocabulary is absent would promise, in the one place that has no capability flag beside it to correct it, precisely the half that is missing. **What a green unit suite here cannot claim** is that a real companion answers as the stub does, or that idb answers in points at all rather than merely consistently with committed fixtures. `tests/device/ios-simulator/read-screen.test.ts` closes that, gated on `ROVER_TEST_SIMULATOR` **and** `ROVER_TEST_IDB` and **read-only** — it boots nothing, launches nothing and taps nothing, so it is safe against a device somebody else is looking at. Its central case asserts the `AXApplication` frame equals `deviceInfo().screen.widthDp`×`heightDp` **exactly**, which catches a conversion in either direction (a division would land at 134×291, a multiplication at 1206×2622) and is strictly stronger than the plan's "every frame inside the screen" — which is not even true, since that Settings row ends 9.67 points below the bottom edge. Beside it: ids unique on a real screen, a second read answered over the companion the first one started, and the refusal by name inside five seconds against a simulator that is not booted. **Phase 5 is done, and with it this row and R46** (#252): the four input primitives and `canInput: true`, which is the last flag on this manifest that was not a permanent opt-out. All four land together because `CAPABILITY_METHODS.canInput` names `tap`, `swipe`, `typeText` **and** `pressKey`, so a manifest declaring the capability with three of them implemented fails the conformance gate — the split point was forced by the repository, exactly as the recording trio's was. **One RPC is behind all four**, because that is what the companion offers: there is no tap call and no text call, only `rpc hid(stream HIDEvent) returns (HIDResponse)`, so the difference between a tap, a swipe, a key and a line of text is entirely which events `src/backends/ios-simulator/input.ts` builds — and `hid` being **client-streaming** is why `idb-client.ts` grew a second call shape beside the unary one, with `IDB_RPCS` split into a unary list and a stream list so naming one in the other's call is a type error. In `@grpc/grpc-js` the options go **before** the callback on a streaming call (`checkOptionalUnaryResponseArguments` reads a leading function as the callback and discards everything after it), so the obvious argument order is a call with no deadline at all — a wedged companion holding a lease open in silence, which is the one thing `IDB_CALL_TIMEOUT_MS` exists to prevent. **Every injection is verified by reading the screen back rather than by a return code**, and that is the phase's whole shape rather than a preference: `hid` answers an empty `HIDResponse` and answered exactly that, at exit 0 and with nothing done, for a keycode of `9999`, a touch at `NaN`, a touch at `(99999, 99999)`, a swipe of `NaN` seconds and a swipe of `-1` seconds (measured), so a suite asserting that the call resolved would be green on a backend that injected nothing. What the checks that *can* be made before the stream is opened therefore are, are made in `input.ts`, where they are loud. **Points go straight through and the absence of a conversion is stated rather than left to be filled in**: idb takes points, which is already `ScreenInfo.widthDp`'s unit and `readScreen`'s, so a `toDevicePixels` analogue copied from the Android side would be the bug rather than the fix — `tests/device/ios-simulator/input.test.ts` is what would catch one, because its tap target is the 20×19-point control inside Spotlight's search field and a ×3 conversion lands off the panel. **`HIDSwipe.duration` is in seconds** where every duration in this contract is milliseconds, so an unconverted number is a swipe a thousand times too long that the companion accepts; measured, the value is honoured with ~100 ms over it (0.05 s → 72 ms, 1.5 s → 1,660 ms), a zero duration is dispatched and moves nothing, and a swipe from a point to itself held 0.8 s **is** the long press, so `src/verbs/input.ts` needs nothing new here. **`typeText` carries idb's keyboard map because the companion has no text RPC at all** — `idb ui text` is a client-side expansion into key presses (`idb/common/hid.py`), transcribed here and verified as a whole: all 95 printable ASCII characters went in one stream and came back out of a text field byte-identical. Outside that set the text is refused **before anything is sent**, as an `UnsupportedTextError` naming the characters, because the alternatives were measured and are both silent — a character with no key produces *no events*, a tab is dropped leaving the field exactly as it was, and a newline is Return, which submits. The numbers as re-measured through this repository's own code on the bench (iPhone 17 / iOS 26.4 / companion v1.5.2, 2026-09-09): `docs/IOS.md` §2's loop **6/6** typed read back exactly and **6/6** taps cleared the field, tap 106–144 ms, the screen read between injections 135–368 ms, `typeText` 102–154 ms for a word and 219–315 ms for all 95 characters, `pressKey('home')` 104–197 ms, a guarded `wake` on an already-woken device 424–557 ms sending nothing at all, and the two per-key refusals in under a millisecond with no round trip. The label finally becomes **`iOS Simulator (simctl + idb)`**, which is the phase after which naming idb promises nothing this backend does not have. | R46 | L |
| R48 | Artifact retention: the policy, its two bounds and the three triggers that run them | **Done** — three phases, and the row that closes §9.4's oldest open entry. Phase 1 (#238) is the policy and the mechanism: `ROVER_ARTIFACTS_BUDGET_MB` and `ROVER_ARTIFACTS_MAX_AGE_DAYS` (`src/daemon/archive-retention.ts`, asserted equal to the System screen's own defaults), the one walk that means them (`src/daemon/archive-sweep.ts`), and the operator's two surfaces — `sweep_archive` and `rover sweep --dry-run|--actor`. What a sweep *is* is D34 (one run directory, taken whole, oldest first by the code-unit order of its name), D35 (a kept test and a live lease's run are exempt from both bounds, absolutely) and D36 (an unmeetable budget is a refusal with one log line, never a kept test deleted). Phase 2 (#245, D37) is the **budget** on the tail of every lease's end, released and expired alike, on the path D9 already runs — behind the release rather than in it, so no agent ever waits for a walk of the archive, and awaited by a shutdown, because a `process.exit` inside the `rm` would leave a run every listing still reports while it holds a subset of what its lease wrote. Phase 3 (#246, D38) is the **clock**: one full pass of both bounds at local midnight and one at daemon start, paced by a comparison against when the pass last ran rather than by the timer, which is what makes a suspended host and a host that was switched off the same case and keeps DST to one pass a day in both directions. The walk's cost was **measured rather than assumed** (§6) because two of the three triggers pay it with nobody asking: ~150 ms for a full archive at the default budget, ~610 ms for one four times over it, and **no cached total was built** — D6 binds it, and the measurement did not call for one | R25, R36, D33 | L |
| R49 | A size badge on every scope of the Archive screen | Three phases. **Phase 1 is done** (#259): the host half — `measure_archive`, one address in and the bytes under it out, with `truncated` when the walk that produced them was cut short. It is `list_archive`'s own path vocabulary and `list_archive`'s own three outcomes, so *nothing is filed here* and *the host cannot say* stay two answers; a directory is walked, a regular file answers its own size, anything else is `unreadable`, and a size the host could not take is **never** a `0`. The primitive is not new and is not copied: `sizeOfTree` moved out of `src/daemon/archive-sweep.ts` into `src/daemon/archive-size.ts` and both call it, gaining only a flag saying a walk was cut short — which is the issue's own condition, that the badge and the sweep's log must not hold two differently-bounded ideas of what the archive weighs. On `PANEL_METHODS` (D29) and not an MCP tool, for `list_archive`'s reason. **Phase 2 is done** (#261): the badge itself on the `All` view — one full sentence naming its own scope, `All tests take 7.7 MB on disk` down to `This file takes 411 KB on disk`, absent rather than `0` where there is nothing to measure, and *at least* wherever the walk was cut short. **Phase 3 is done** (#262): the group scopes, as a second method beside the first — `measure_archive_groups`, three scopes (everything grouped, one project's grouped runs, one group's), `MeasureArchiveResultSchema` reused unchanged, its walk `list_archive_groups`' walk with `list_archive_groups`' bounds, each run's `group_id.json` read through that module's own `readGroupId` and each matching run added through the same `sizeOfTree` — which is the `list_archive` / `list_archive_groups` precedent and never a parameter. The groups view draws it at every depth: `Grouped tests take …` at its root, `Grouped tests in this project take …` under a project, `Tests in this group take …` at a group, and below a group exactly the `All` view's own sentences, because the address there is the archive's own once the group id is dropped. **No grouped scope says *all***, because that view lists only the runs that named a `group_id` and a subset described with the word *all* is the one thing this badge must not do; and a grouped total whose bounded walk was cut short renders a lower bound rather than a plain figure, which is why a bounded walk may carry a size badge where it may not carry a count badge. **And the `archive` scope has a second reader** (#260): the System screen's `ARCHIVE SETTINGS` card draws it in its own header strip, from `HeaderBadge` and `sizeSentence` unchanged, so the panel says what the archive weighs on the screen where its budget is typed. Nothing on the host changed for it, and it is deliberately **not** an *`X of Y`* — no answer carries the budget, so the field beside it is a draft nobody has saved and a comparison would present it as the bound the sweep enforces. There is still no index: the walk *is* the measurement (D6, D24) | R36, R48 | M |
...
- **Retention policy for the artifact archive — closed, and moved into the backlog proper as R48
  ...
  had to be settled before it could be filed, is most of what it is worth now.

  **What this said, and why it was right at the time.** A TTL, a size cap, and who runs the prune
  — a human operator by cron, or the daemon itself — were all undecided, and a row filed against
  any of the three would have been a row filed against a guess. What it also said, correctly, was
  that this could not be left until disk pressure was actually observed: once a panel read this
  archive directly, unbounded growth was a problem from that screen's first day. So the
  constraint it carried was an **ordering** one — R25 builds the archive with no pruning of its
  own, and the retention row is filed before the first screen that browses it. That held: the
  narrowing on 2026-08-31 (the Devices screen reads the live inventory and cannot make growth
  worse) bought the delay honestly, and the exemption landing first on 2026-09-08 (#234, D33) was
  the other deliberate piece of the order — a sweep must not ship before there is a way to say
  *not this test*, because it could then delete a run an operator had every reason to believe was
  safe.

  **All three are settled now, and the mechanism is built.** The **cap** is
  `ROVER_ARTIFACTS_BUDGET_MB` (1024) and the **TTL** is `ROVER_ARTIFACTS_MAX_AGE_DAYS` (30), both
  host environment variables held equal to the System screen's own defaults by a test. The
  **prune's owner** turned out to be three owners rather than one, which is why the question read
  as harder than it was: an **operator** (`sweep_archive`, `rover sweep`), a **lease's end** for
  the budget alone (D37), and a **clock** for the whole policy at local midnight and at daemon
  start (D38). No cron and no operator discipline anywhere — a host nobody types a command on
  enforces both of its bounds, which is what this row was named after and the condition on which
  it closes.

  **So it belongs in §9.3 rather than here**, as R48, naming its three phases (#238, #245, #246).
  Nothing about retention is outside the backlog any more.
...
- **And it answers *how much*, which is the archive's fourth read** — `measure_archive` (R49),
  since 2026-09-09. It takes one address — the same component array `list_archive` takes, `[]`
  being the root — and answers the bytes under it with a flag saying whether the walk that produced
  them was cut short. A directory is walked, a regular file answers its own size, and an address
  that is neither is `unreadable`; **a size the host could not take is never a `0`**, because `0` is
  a true claim about an empty directory and the pair must never render alike. It is a **separate
  method rather than a parameter** on `list_archive`, whose shape is untouched, and it shares the
  **sweep's own** `sizeOfTree` (`src/daemon/archive-size.ts`, moved there from
  `src/daemon/archive-sweep.ts` and used by both) so a figure on a screen and a figure in the
  sweep's log can never be two differently-bounded ideas of what this tree weighs. They are not
  arithmetically the same number and this section says so rather than implying it: the sweep totals
  *run subtrees*, so a stray file beside a project counts for a measurement and not for the budget.
  There is still **no index** — the walk *is* the measurement (D6, D24), which is why the host's
  depth bound is the bound and why a bounded answer says `truncated`. No host path and no `errno` is
  on any answer and there is no field either would fit in (D19). On `PANEL_METHODS` (D29) and
  deliberately **not** an MCP tool, for `list_archive`'s reason: how much disk an operator's archive
  takes is that operator's browser's question.
- **And it answers *how much of it is grouped*, which is the archive's fifth read** —
  `measure_archive_groups` (R49), since 2026-09-09. It is the row above's question over the three
  scopes an *address* cannot name: everything grouped, everything grouped in one project, and one
  group. Two of those describe a **subset** of this tree — the runs that named a `group_id` — and
  the third has no directory to address either, because there is no `<group_id>/` level and never
  was (R41). So it is a **second method beside the fourth rather than a parameter on it**, which is
  the `list_archive` / `list_archive_groups` precedent exactly, and the two walks are genuinely
  different: a subtree sum against a walk that reads `group_id.json` per run. That walk is
  `list_archive_groups`' own, with its bounds and its `MAX_ARCHIVE_GROUP_DIRECTORIES`, reading each
  run's claim through that module's exported `readGroupId` so the two answers cannot disagree about
  what a group id is on disk; and every matching run adds the **run directory** through the same
  `sizeOfTree` the fourth method and the sweep use, so a group's total, an address's total and the
  sweep's log are one primitive with one bound. It answers `MeasureArchiveResultSchema` unchanged.
  `truncated` is set by the directory bound, by a level the host could not read, by a
  `group_id.json` that will not parse — that run *is* grouped, so an incomplete total must not
  render as a complete one — and by any run whose own subtree walk came back short. **A scope that
  matches no run is `measured` with `bytes: 0`**, because *nothing grouped here* is a true claim
  about zero bytes and is not a failed `stat`; the scope's own root absent is `missing` and
  unreadable is `unreadable`, which for two of the three scopes is the project's directory. No host
  path and no `errno` is on any answer and there is no field either would fit in (D19). On
  `PANEL_METHODS` (D29) and deliberately **not** an MCP tool, for the fourth row's reason with
  `list_archive_groups`' force: an agent already knows its own group, having chosen it.
- **The operator can now say *keep this test* — and the tree grew nothing to carry it** (D33,
  ...
  argument is D33.
- **Retention now has a policy, and this tree is what it deletes from** (R48, D34–D38, #238,
  #245, #246).
  ...
  **Both bounds run on their own, and there are three triggers.** Every lease that ends — released
  and expired alike — is followed by a **budget-only** sweep of this tree (D37, #245), because a run
  finishing is the one moment its *size* can be newly crossed. The **whole** policy runs on a
  clock: one pass at local midnight and one at daemon start (D38, #246), paced by a comparison
  against when the pass last ran rather than by the timer, so a host that was asleep or switched
  off at midnight sweeps as it comes up. And an operator asks for the whole policy through
  `rover sweep` and `sweep_archive`. So nothing about this tree waits for somebody to remember it,
  and the worst case is that it stands over one of its bounds for up to a day (D38). *This
  paragraph said the age bound had no trigger but a command until #246 closed §9.4's row; the
  budget's half is unchanged.*
