...
- **The tree is now *read* over the surface** — `list_archive` (R36), one directory level per call,
  ...
  host cannot read is diagnosed in the host's own log instead. It is a listing and not a query, for
  D24's reason: the parameter that would make it one is how an index gets built by accident.
