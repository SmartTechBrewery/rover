...
`record`, `pull`, `push`,
`install`, `archive`, `keep`, `sweep`, `delete-project`, `delete-test`, `status` and `users`, human-readable by default and one JSON document on stdout with `--json`, every diagnostic
...
and the same four answers mean the same four next moves. `sweep`
...
refused because a lease filing into it is live, an unreachable host, a
