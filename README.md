...
- **Only half the host's retention policy runs on its own.** The **disk budget** is enforced after every lease ends, released or expired alike, so `~/.rover/artifacts` no longer grows past `ROVER_ARTIFACTS_BUDGET_MB` on a host nobody types a command on (`PROJECT.md` D37, §9.4). The **age limit** has no such trigger — no timer, no midnight pass, no pass at start-up — so a test on a host that is inside its budget can sit there past `ROVER_ARTIFACTS_MAX_AGE_DAYS` until somebody runs `rover sweep`.
...
and deletes nothing (`PROJECT.md` §9.4).
...
is that policy's one trigger: it deletes the runs the host's disk budget and age limit no longer
keep — oldest first, a whole run at a time — with `--actor` required and `--dry-run` to ask what
would go and delete nothing. **Nothing on the host runs it on its own** (`PROJECT.md` §9.4).
...
| `ROVER_ARTIFACTS_PATH` | `~/.rover/artifacts` | Root of the durable artifact archive: every `screenshot`, `record_video`, `stop_recording` and `read_logs` call additionally writes its output here, on the host, **in addition to** returning the bytes to the client (`PROJECT.md` D23, §10). **Empty counts as unset**, as it is for the socket. Read only by the daemon — a client never resolves it, and the archive path is never the one an agent is given. **Something prunes it now**: `rover sweep` runs the host's whole retention policy over this tree — `ROVER_ARTIFACTS_BUDGET_MB` and `ROVER_ARTIFACTS_MAX_AGE_DAYS` below — deleting whole run directories, oldest first, and the **budget** half of it additionally runs after every lease ends with nobody asking (`PROJECT.md` D37). So this no longer grows past its budget on its own; what still waits for an operator is the age limit (`PROJECT.md` §9.4). |
...
The host will delete what its retention policy no longer keeps. It enforces the **disk budget**
after every lease ends without being asked (below); this is how you run the **whole** policy, the
age limit included, and how you ask what it *would* take first:
...
**The host runs the budget half of this on its own, and only that half.** After every lease ends
— released and expired alike — it sweeps by `ROVER_ARTIFACTS_BUDGET_MB` with nobody asking
(`PROJECT.md` D37), behind the release rather than inside it, so a release is never slowed or
failed by one. The **age limit** has no trigger but this command: no timer, no midnight pass, no
pass at start-up (`PROJECT.md` §9.4).
