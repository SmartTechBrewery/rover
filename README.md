...
operator naming one project through `rover delete-project` is not one of them, so an explicit delete
takes a kept test with the rest of that project's subtree and its entry goes with it, and the answer
says how many went (`PROJECT.md` D35, D42).
...
`record`, `pull`, `push`,
`install`, `archive`, `keep`, `sweep`, `delete-project`, `status` and `users`, human-readable by default and one JSON document on stdout with `--json`, every diagnostic
...
Four answers, four next moves, and never a success that removed nothing. `sweep`
...
project is live, an unreachable host, a
